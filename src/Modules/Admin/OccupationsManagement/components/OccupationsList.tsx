...
import { useOccupationsListController, SortField, SortDirection } from '../controllers/OccupationsListController';

interface OccupationsListProps {
  onEdit: (occupation: OccupationModel) => void;
  onDelete: (id: number) => void;
}

export const OccupationsList: React.FC<OccupationsListProps> = ({ onEdit, onDelete }) => {
  const { state, controller, filteredOccupations, paginatedOccupations, totalPages } = useOccupationsListController();
  ...
  const handleDelete = async (id: number, name: string) => {
    if (confirm(`Are you sure you want to delete "${name}"?`)) {
      const success = await controller.deleteOccupation(id);
      if (success) {
        onDelete(id);
      } else {
  ...
  if (state.loading) {
    return (
      <Card className="flex items-center justify-center h-64">
        <LoadingSpinner size="lg" text="Loading occupations..." />
      </Card>
    );
  }
              ...
              {paginatedOccupations.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                    {state.searchQuery ? 'No occupations found matching your search.' : 'No occupations found. Create your first occupation to get started.'}
                  </td>
                </tr>
              ) : (
                paginatedOccupations.map((occupation) => (
                  <tr key={occupation.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleDelete(occupation.id, occupation.name)}
