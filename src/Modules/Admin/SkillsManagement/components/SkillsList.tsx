...
import { useSkillsListController, SortField, SortDirection } from '../controllers/SkillsListController';
...
export const SkillsList: React.FC<SkillsListProps> = ({ onEdit, onDelete }) => {
  const { state, controller, filteredSkills, paginatedSkills, totalPages } = useSkillsListController();
  ...
  const handleDelete = async (id: number, name: string) => {
    if (confirm(`Are you sure you want to delete "${name}"?`)) {
      const success = await controller.deleteSkill(id);
      if (success) {
        onDelete(id);
      } else {
              ...
              {paginatedSkills.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                    {state.searchQuery ? 'No skills found matching your search.' : 'No skills found. Create your first skill to get started.'}
                  </td>
                </tr>
              ) : (
